...
/*
 ...
 original version used a property called 'spokenMessage' to speak something.
 So now if your new overridden speak() method does not use that property, it
 will be programmed to speak something that isn't consistent with the
 spokenMessage that is stored as a state property. If that message
 consistently needs to be maintained, you are going to have a problem.

 Also, notice that in the Animal class version of speak(), a JOptionPane is
 used to display what is spoken. But in the overridden versions (see Duck
 and Dog) the console is used. Does it matter? It might, depending on the
 type of GUI you might be using.

 All of these problems could have been avoided by declaring an abstract
 speak(String message) method that is documented as needing to copy the
 message argument into the spokenMessage property. This would be accomplished
 via a set of concrete methods: setSpokenMessage() and getSpokenMessage(). As
 you can see, implementation inheritance is not always the best choice.
 */
public class Main {
    ...
    public static void main(String[] args) {
        ...
        // Or is it? What if I don't want output going to a JOptionPane?
        mute.speak();
